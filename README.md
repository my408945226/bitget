...
graph TD
    ...
    J --> K[检查订单成交]
    K --> L{SELL 成交?}
    L -->|是| M[stack_top 上移<br/>opens++<br/>重挂网格]
    L -->|否| N{BUY 成交?}
    N -->|是| O[closes++<br/>清除所有 BUY<br/>重挂完整梯队]
    N -->|否| P[Watchdog 检测]
    P --> Q{价格穿挂单?>10s}
    Q -->|是| R[市价降级成交]
    Q -->|否| J
...
**问题**：震荡市中价格可能短暂穿过挂单后立即回归，如果立即市价降级会产生高额滑点。

**解决方案**：价格穿过挂单需持续 **10 秒** 才触发市价降级。

**工作流程**：
```
时刻 T0: 价格穿过 SELL 挂单 → 记录时间 watchdog_sell_start_ts = T0
时刻 T1: 价格仍在挂单上方 → 检查 (T1 - T0) >= 10s ?
  - 是 → 触发市价加仓
  - 否 → 继续等待
时刻 T2: 价格回归正常 → 重置 watchdog_sell_start_ts = 0
```

**可调整参数**（修改 `strategy.py`）：
```python
self.WATCHDOG_CONFIRM_SECONDS = 10  # 改为 5 或 15 秒
```
...
**触发条件**：
1. 价格穿过 SELL 挂单 ≥ 10 秒 → 市价加仓
2. 价格穿过最高 BUY 挂单 ≥ 10 秒 → 市价平仓
3. postOnly 下单被拒（价格已穿）→ 立即市价降级

**避免频繁触发**：
- 增大 `--grid` 参数（挂单间距更大）
- 增加 `WATCHDOG_CONFIRM_SECONDS`（延长确认时间）
- 选择波动率适中的交易对
...
**最后更新时间**：2026-06-14  
**版本**：v2.0.0（Maker 优先版 + 双层风控 + Telegram 通知）
